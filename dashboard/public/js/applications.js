...
async function openEditor(typeId) {
  ...
  ED.innerHTML = `
    ...
    <form class="card" id="typeForm">
      <div class="card__head"><h2>${icon('settings')} Allgemeines</h2></div>
      <div class="form">
        <div class="col-2">
          <div class="field"><label>Name</label><input name="name" value="${escapeHtml(t.name)}" /></div>
          <div class="field"><label>Emoji</label><input name="emoji" value="${escapeHtml(t.emoji || '')}" maxlength="8" data-emoji="one" /></div>
        </div>
  ...
  Dash.trackForm(typeForm, async () => {
    try {
      await apiFor('PATCH', `/application-types/${t.id}`, readForm(typeForm));
      toast('Gespeichert.', 'success');
...
async function renderQuestions() {
  ...
  wrap.innerHTML = qs.length ? qs.map((q, i) => `
    <form class="q-item" data-q="${q.id}">
      ...
      <button type="button" class="btn btn--ghost btn--sm" data-a="down" ${i === qs.length - 1 ? 'disabled' : ''}>↓</button>
      <button type="button" class="btn btn--danger btn--sm" data-a="del">${icon('trash', 'icon--sm')}</button>
    </form>`).join('')
